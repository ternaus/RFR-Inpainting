...
class AttentionModule(nn.Module):
    def __init__(self, inchannel, patch_size_list=[1], propagate_size_list=[3], stride_list=[1]):
        if not isinstance(patch_size_list, list):
            raise ValueError(
                "patch_size should be a list containing scales, "
                "or you should use Contextual Attention to initialize your module"
            )

        if not len(patch_size_list) == len(propagate_size_list) and len(propagate_size_list) == len(stride_list):
            raise ValueError("the input_lists should have same lengths")
