...
class PartialConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        self.slide_winsize = (
            self.weight_maskUpdater.shape[1] * self.weight_maskUpdater.shape[2] * self.weight_maskUpdater.shape[3]
        )
        ...
        if mask is not None or self.last_size != (input.data.shape[2], input.data.shape[3]):
            ...
            with torch.no_grad():
                ...
                if mask is None:
                    ...
                    if self.multi_channel:
                        mask = torch.ones(
                            input.data.shape[0], input.data.shape[1], input.data.shape[2], input.data.shape[3]
                        ).to(input)
                    else:
