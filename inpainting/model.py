...
from inpainting.modules.RFRNet import RFRNet, VGG16FeatureExtractor
from inpainting.utils.io import load_ckpt, save_ckpt
...
class RFRNetModel:
    ...
    def train(self, train_loader, save_path, finetune=False):
        ...
        keep_training = True
        print(f"Starting training from iteration:{self.iter:d}")
        s_time = time.time()
        while keep_training:
            for items in train_loader:
                ...
                if self.iter % 40000 == 0:
                    if not os.path.exists(f"{save_path:s}"):
                        os.makedirs(f"{save_path:s}")
                    save_ckpt(
                        f"{save_path:s}/g_{self.iter:d}.pth",
                        [("generator", self.G)],
    ...
    def test(self, test_loader, result_save_path):
        ...
        for items in test_loader:
            ...
            if not os.path.exists(f"{result_save_path:s}/results"):
                os.makedirs(f"{result_save_path:s}/results")
            for k in range(comp_B.size(0)):
                count += 1
                grid = make_grid(comp_B[k : k + 1])
                file_path = f"{result_save_path:s}/result_final/img_{count:d}.png"
                save_image(grid, file_path)

                grid = make_grid(masked_images[k : k + 1] + 1 - masks[k : k + 1])
                file_path = f"{result_save_path:s}/result_final/masked_img_{count:d}.png"
                save_image(grid, file_path)
    ...
    def get_g_loss(self):
        ...
        preceptual_loss = self.preceptual_loss(real_B_feats, fake_B_feats) + self.preceptual_loss(
            real_B_feats, comp_B_feats
        )
        valid_loss = self.l1_loss(real_B, fake_B, self.mask)
        hole_loss = self.l1_loss(real_B, fake_B, (1 - self.mask))

        loss_G = tv_loss * 0.1 + style_loss * 120 + preceptual_loss * 0.05 + valid_loss * 1 + hole_loss * 6
